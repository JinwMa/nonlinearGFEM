...
void SPC::takeDB(Input *pinput, Mesh *pmesh)
{
    // 读节点
    std::string nodes_type = pinput->getString(name + "_nodes_type");
    ...
    // 读自由度
    d_dof = pinput->getString(name + "_dof");

    // 读值
    if (pinput->ifExist(name + "_value"))
    {
        d_is_value = true;
        d_value = pinput->getDouble(name + "_value");
    }
    else if (pinput->ifExist(name + "_value_expression"))
    {
        d_is_value_expression = true;
        d_value_expression = pinput->getVectorDouble(name + "_value_expression");
    }
    else
    {
...
void SPC::getNodesFromGeometry(Mesh *pmesh)
{
    ...
    if (count == 0)
        toolbox::error("the size of d_geometry_expression is wrong 0");
    if (count > 10)
        toolbox::error("the size of d_geometry_expression is wrong 10");
    ...
    for (int i = 0; i < node_num; i++)
    {
        ...
        if (std::fabs(value) < eps)
            d_node_ids.push_back(node_id);
    }
}

std::vector<ConstraintEquation> SPC::buildEquations(Mesh *pmesh)
{
